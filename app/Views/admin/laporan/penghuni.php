...
<div class="print-header">
    ...
    <div style="text-align: right;">
        <h3 style="margin:0; font-weight: bold;">LAPORAN DATA PENGHUNI</h3>
...
<div class="card border-0 shadow-sm mb-3 no-print">
    <div class="card-body d-flex justify-content-between align-items-center py-3">
        ...
        <div class="d-flex gap-2">
            <a href="/admin/laporan/export-penghuni" class="btn-export"><i class="bi bi-file-earmark-excel"></i> Export Excel</a>
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-primary text-white text-center py-3 no-print">
        <h5 class="mb-0 fw-bold"><i class="bi bi-people me-2"></i>LAPORAN DATA PENGHUNI</h5>
    ...
    <div class="card-body">
        <div style="display: flex; flex-direction: row; flex-wrap: nowrap; gap: 8px; margin-bottom: 15px; width: 100%;">
            <div style="flex: 1 1 33.33%; max-width: 33.33%; min-width: 0;">
                <div class="stat-box" style="background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 8px; padding: 12px; text-align: center;">
                    <div style="font-size: 26px; font-weight: bold; color: #0d6efd; margin-bottom: 2px;"><?= (string)$total ?></div>
                    <small style="color: #6c757d; font-size: 11px;">Total Penghuni</small>
                </div>
            </div>
            <div style="flex: 1 1 33.33%; max-width: 33.33%; min-width: 0;">
                <div class="stat-box" style="background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 8px; padding: 12px; text-align: center;">
                    <div style="font-size: 26px; font-weight: bold; color: #198754; margin-bottom: 2px;"><?= (string)$aktif ?></div>
                    <small style="color: #6c757d; font-size: 11px;">Penghuni Aktif</small>
                </div>
            </div>
            <div style="flex: 1 1 33.33%; max-width: 33.33%; min-width: 0;">
                <div class="stat-box" style="background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 8px; padding: 12px; text-align: center;">
                    <div style="font-size: 20px; font-weight: bold; color: #fd7e14; margin-bottom: 2px;">Rp <?= number_format((int)$total_deposit,0,',','.') ?></div>
                    <small style="color: #6c757d; font-size: 11px;">Total Deposit</small>
                </div>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered align-middle">
                <thead class="table-dark text-center">
                    <tr><th>No</th><th>Nama</th><th>Kamar</th><th>Mulai Huni</th><th>Selesai</th><th>Durasi</th><th>Deposit</th><th>Status</th></tr>
                </thead>
                <tbody>
                    <?php $no=1; foreach ($penghuni as $p): ?>
                    <tr>
                        <td class="text-center"><?= $no++ ?></td>
                        <td><strong><?= esc($p['nama']) ?></strong><br><small class="text-muted"><?= esc($p['no_hp'] ?? '') ?></small></td>
                        <td class="text-center"><strong>No. <?= esc($p['nomor_kamar']) ?></strong></td>
                        <td class="text-center"><small><?= esc(date('d M Y', strtotime($p['tanggal_mulai'] ?? 'now'))) ?></small></td>
                        <td class="text-center"><small><?= esc(date('d M Y', strtotime($p['tanggal_selesai'] ?? 'now'))) ?></small></td>
                        <td class="text-center"><span class="badge bg-info text-dark"><?= esc($p['durasi_bulan']) ?> bln</span></td>
                        <td class="text-end">Rp <?= number_format($p['deposit'] ?? 0,0,',','.') ?></td>
                        <td class="text-center">
                            <?php $badge = ['menunggu'=>'warning text-dark','aktif'=>'success','ditolak'=>'danger','selesai'=>'secondary']; $b = $badge[$p['status']] ?? 'secondary'; ?>
                            <span class="badge bg-<?= $b ?>"><?= esc(ucfirst($p['status'])) ?></span>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
